var log = require('okdoki/lib/base').log;
var SQL       = require('okdoki/lib/SQL').SQL;
var _         = require('underscore');
var pg        = require('pg');
var uri       = require('uri-js');

var PG = exports.PG = function () {};
PG.id_size = 10;
PG.show_default_owner_sql = "SELECT usename AS owner \
FROM pg_database, pg_user \
WHERE datname = current_database() AND datdba = usesysid;";
PG.show_databases_sql = "SELECT datname AS name \
FROM pg_database WHERE datistemplate = false AND datname LIKE 'custom%';";

PG.rollback = function rollback(client, on_err) {
  return function (err, meta) {
    if (err) {
      client.query('ROLLBACK', [], function (new_err) {
        client.end();
        if (on_err)
          on_err(new_err || err);
        else
          throw (new_err || err);
      });
    };

  };
};

var Client = function (db) {

  var db_url = process.env.DATABASE_URL;

  if (db) {
    var pieces = uri.parse(process.env.DATABASE_URL);
    pieces.path = db;
    db_url = uri.serialize(pieces);
  }

  var c = new pg.Client(db_url);
  c.connect();

  return c;
};

Client.new = function (db) {
  return Client(db);
}


PG.new = function (name, flow) {
  var pg = new PG();

  _.each(arguments, function (v) {
    if (_.isString(v) && !pg.name)
      pg.name = v;
    else if (v.finish && !pg.flow)
      pg.flow = v;
  });

  pg.sql         = null;
  pg.querys      = [];
  pg.finishs     = [];
  pg.replys      = [];
  pg.dups        = {};
  pg.error_funcs = [];
  return pg;
};

PG.run = function (flow, q) {
  return PG.new(flow.group + ':' + flow.id, flow)
  .q(q)
  .run(flow.finish);
};

PG.prototype.toString = function () {
  if (this.name)
    return "PG: " + this.name;
  return "PG (instance)";
};

PG.prototype.error = function (msg) {
  if (this.flow)
    return flow.error(msg);
  throw new Error(msg);
};

PG.prototype.delete_trashed = function (table_name) {
  var sql = "DELETE FROM " + table_name + " \n" +
    "WHERE trashed_at IS NOT NULL AND  \n" +
    "(extract(epoch from ( timezone('UTC'::text, now()) - trashed_at)) / 60 / 60) > 48 \n" +
    "RETURNING * ;";
  this.q(sql);
  return this;
};

PG.show_tables = function () {
  var sql  = PG.show_tables.sql
  , on_fin = null
  , flow   = null;

  _.each(arguments, function (v) {
    if (_.isString(v))
      sql += ' ' + v;
    else if (_.isFunction(v))
      on_fin = v;
    else
      flow = v;
  });

  var db = PG.new().q(sql).run(function (rows) {
    var tables = _.pluck(rows, 'name');
    if (on_fin)
      return on_fin(tables);
    if (flow)
      return flow.finish(tables);
  });

  return db;
};

// FROM: http://stackoverflow.com/questions/769683/show-tables-in-postgresql
PG.show_tables.sql = "SELECT table_schema || '.' || table_name AS name\
  FROM    information_schema.tables         \
  WHERE   table_type = 'BASE TABLE'         \
  AND     table_schema NOT IN ('pg_catalog', 'information_schema')";


PG.prototype.q = function (v, arr) {
  if (v.to_sql)
    v = v.to_sql();
  else if(arguments.length === 1 && _.isString(v))
    v = [v, []];
  else if(arguments.length === 1 && _.isArray(v) && v.length == 2)
    v = v;
  else if(arguments.length === 2 && _.isString(v) && _.isArray(arr))
    v = [v, arr];
  else if(arguments.length === 1 && v.sql && v.vals)
    v = v;
  else
    return this.error('Unknown arguments:' + arguments);
  this.querys.push(v);

  return this;
};

PG.prototype.on_dup = function (name, f) {
  if(!this.dups[name])
    this.dups[name] = [];
  this.dups[name].push(f);
  return this;
};

PG.prototype.delete_all = function (name) {
  this.querys.push(['DELETE FROM ' + name + ' ;']);
  return this;
};

PG.prototype.insert_into = function (tname, vals) {
  var me = this;
  me.last_insert = SQL.new().insert_into(tname);

  if (vals) {
    me.q(me.last_insert.values(vals));
    me.last_insert = null;
  }

  return me;
};

PG.prototype.values = function (vals) {
  var me = this;
  me.q(me.last_insert.values(vals));
  return me;
};

PG.prototype.update = function (name) {
  var me = this;
  me.curr_sql = SQL.update(name);
  return me;
};

// _.each('select delete'.split(' '), function (name) {
  // PG.prototype[name] = function () {
    // this.sql = SQL.new();
    // this.sql[name].apply(this.sql, arguments);
    // this.querys.push([this.sql]);
    // return this;
  // };
// });

_.each("set from where limit".split(' '), function (name) {
  PG.prototype[name] = function () {
    var me = this;
    me.curr_sql[name].apply(me.curr_sql, arguments);
    return me;
  };
});

PG.prototype.error = function (msg) {
  if (this.flow)
    this.flow.error(msg);
  else
    throw new Error(msg);
};

PG.prototype.reply = function (f) {
  this.querys.push(f);
  return this;
};

PG.prototype.run = function (f) {
  var me = this;

  if (me.curr_sql) {
    me.q(me.curr_sql)
    delete me.curr_sql;
  }

  if (arguments.length > 1) {
    var msg = 'Extra arguments not allowed: ' + _.toArray(arguments).slice(1).toString();
    if (this.flow)
      return this.flow.error(msg);
    throw new Error(msg);
  }

  if (f)
    this.on_finish(f);

  var next_q = this.querys.shift();

  if (!next_q) {
    me.client && me.client.end();
    return me.finish();
  }

  if (!me.client)
    me.client = Client.new();

  if (_.isArray(next_q)) {
    var sql = next_q[0], vals = next_q[1] || [];
    var q = {sql: sql, vals: vals};
  } else if (_.isFunction(next_q)) {
    this.replys.push( next_q(_.last(this.replys)) );
    return this.run();
  } else {
    var sql = next_q.sql, vals = next_q.vals;
    var q = next_q;
  }

  me.client.query(sql, vals, function (err, meta) {
    me.finish(err, meta, q);
  });

};

PG.prototype.on_error = function (f) {
  this.error_funcs.push(f);
  return this;
};

PG.prototype.on_finish = function (f) {
  this.finishs.push(f);
  return this;
};

PG.prototype.finish = function (err, meta, q) {
  var me = this;

  if (arguments.length === 0 && me.querys.length === 0) {

    var result = _.last(me.replys);

    if (!me.finishs.length && me.flow) {
      return me.flow.finish(result);
    }

    _.each(me.finishs, function (f) {
      f(result, me);
    });

    return;
  }

  if (err) {
    var dup_f_arr = _.find(me.dups, function (arr, name) {
      if (!err.detail)
        return false;

      if (err.detail.indexOf("Key (" + name + ")=") > -1 &&
           err.detail.indexOf(") already exists") > 0)
        return arr;

      if (err.detail.indexOf("duplicate key value violates unique constrait") > -1 &&
              err.detail.indexOf("_" + name + '_') > 33)
        return arr;

      return false;
    });

    if (dup_f_arr && dup_f_arr.length) {
      _.each(dup_f_arr, function (f, i) {
        f(err, me);
      });
      return false;
    }

    if (process.env.IS_TESTING && !me.error_funcs.length)
      console['log'](q.sql);

    _.each(me.error_funcs, function (f) {
      f(err, meta, me);
    });

    if (me.flow)
      return me.flow.error(err, meta);

    if (me.error_funcs.length)
      return false;

    throw err;
  }

  var p_key = 'pass_phrase_hash';
  if (meta.rows && meta.rows[0] && _.contains(_.keys(meta.rows[0]), p_key)) {
    _.each(meta.rows || [], function (row, i) {
      delete row[p_key];
    });
  }

  if (q && (q.row_1 || q.limit_1)) {
    if (!meta.rows.length && me.flow)
      return me.flow.not_found('Not found: ' + (_.isNumber(me.flow.id) ? me.name : me.flow.id));
    me.replys.push(meta.rows[0]);
  } else
    me.replys.push(meta.rows);

  me.run();
};



// ****************************************************************
// ****************** OLD CODE ************************************
// ****************************************************************


