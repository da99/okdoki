...
function Query_To_WHERE(data, table_name) {
  ...
  _.each(data.where, function (v, i) {
    v = v.trim();
    if (v.indexOf('$') === -1 && data.vals[i].length === 1)
      v = v + ' = $1 '
...
function Query_To_SQL(sql_query) {
  ...
  if (data.insert_values.length) {
    ...
    sql.push('VALUES ( ' + _.map(data.insert_values, function (p, i) { return '$' + ( i + 1 ); }).join(', ') + ' )');
    sql.push('RETURNING * ');
    meta.vals = _.pluck(data.insert_values, 1);
  }
