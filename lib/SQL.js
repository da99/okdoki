...
_.each('select insert_into delete_from update'.split(' '), function (name) {
  SQL[name] = function () {
    var sql = SQL.new();
...
function Query_To_SQL(sql_query) {
  ...
  if (data.insert_values.length) {
    ...
    sql.push('VALUES ( ' + _.map(data.insert_values, function (p, i) {
      var key = p[0], v = p[1];
      var $n = '$' + ( i + 1 );
      if (_.isArray(v) && v.length === 2) {
        meta.vals.push(v[1]);
        return v[0].replace(/\$x/g, $n);
      }
  ...
  return {
    ...
    limit_1   : sql_query.is_insert || _.last(data.limit) === 1
  };
}
