...
SQL.new = function () {
  var sql = Table.new.apply(Table, arguments);
  return sql;
};
...
Statement.new = function () {
  var s = new Statement();
  return s;
};

Statement.prototype.from = function (t) {
  this.d.from.push(t);
  return this;
};

Statement.prototype.join = function (j) {
  this.d.join.push(j);
  return this;
};
...
function Table_To_SQL(sql_table) {
  var sql  = [];
  var vals = [];
  var table = sql_table.data();
  var table_name = sql_table.table_name;

  if (table.select.length) {
    sql.push( 'SELECT ' + add_table_name_to_each(table_name || '', table.select)
    .join(', ') );
  }

  if (table.from.length) {
    sql
    .push( 'FROM ' + _.inject(table.from, function (memo, v) {
  ...
  var l = 0;
  _.each(table.where, function (v, i) {
    var sql_sub = add_table_name(table_name, v).replace(/\$[0-9]+/g, function (sub) {
      var num = parseInt( sub.replace('$', '') );
      return '$' + (num + l);
    });
    vals = vals.concat(table.vals[i]);
    l = vals.length;
    sql.push( sql_sub );
  });
...
var Table = function (parent) {
  ...
  this.d.vals   = [];
  this.d.join   = [];
  this.d.statement = Statement.new().from(this);
  this.is_table = true;
};

Table.new = function (parent) {
  var t = new Table(parent);
  return t;
};
...
Table.prototype.from = function (o) {
  this.d.from.push(o);
...
Table.prototype.where = function (sql, vals) {
...
Table.prototype.and = function (sql, vals) {
  this.where(' AND ' + sql, vals);

  return this;
};

Table.prototype.or = function (sql, vals) {
...
_.each('left right inner'.split(' '), function (name) {
  name = name || 'inner';
  Table.prototype[name + '_join'] = function (sql) {
...
Table.prototype.to_sql = function () {
  return Table_To_SQL(this);
};
...
Join.prototype.to_sql = function (name) {
  ...
  if (me.d.sql) {
    ons = ons.concat(Table_To_WHERE(me.d.sql)[0]);
  }
  sql += ons.join(' AND ') +  ' )';
