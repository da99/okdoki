...
S.create = function (customer, job) {
  ...
  .job('create', 'screen name', function (j) {
    ...
    Arango.new(TABLE_NAME).create(insert_data, j);
...
S.read_by_id = function (id, flow) {
  ...
  .job('read screen name id:', id, function (j) {
    Arango.new(TABLE_NAME).read_one_by_example({id: id}, j);
  })
  .reply(function (r) {
    return S.new(r);
  })
...
S.read_by_screen_name = function (n, flow) {
  ...
  .job('read screen name:', n, function (j) {
    Arango.new(TABLE_NAME).read_one_by_example({screen_name: n.toUpperCase()}, j)
  })
  .reply(function (r) {
    return S.new(r);
  })
...
S.read_list = function (c, flow) {
  ...
  .job('read screen names', c.data.id, function (j) {
    Arango.new(TABLE_NAME).read_by_example({owner_id: j.id}, j)
  })
  .reply(function (r) {
...
S.update = function ( customer, flow ) {
  ...
  .job('update screen name', me.data.screen_name, function (j) {
    Arango.new(TABLE_NAME).update(row.id, set, j)
  })
  .reply(function (row) {
...
S.untrash = function (id, river) {
  River.new(arguments)
  .job('untrash:', id, function (j) {
    Arango
    .new(TABLE_NAME)
    .untrash(id, j);
  })
...
S.delete_trashed = function (flow) {
  River.new(arguments)
  .job('get ids', function (j) {
    Arango.new(TABLE_NAME)
    .del_list("FOR s in " + TABLE_NAME
               + " FILTER s.trashed_at > 0 && s.trashed_at < @two_days LIMIT 20 RETURN s.id",
               {two_days: (new Date).getTime() - (1000 * 60 * 60 * 48)},
              j)
  })
  .job(function (j) {
    var ids = j.river.last_reply();
    var sub = River.new(j);
    _.each(ids, function (id) {
      sub.job('delete', id, function (sub_j) {
        Arango.new(TABLE_NAME).del(id, sub_j);
      });
    });
    sub.run();
  })
