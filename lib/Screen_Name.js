...
VC.define('screen_name', function (v) {
  ...
  _.each(banned_screen_names, function (v, i) {
    v.not_match(new RegExp(v, 'ig'), 'Screen name not allowed: ' + v.val());
  });
});
...
S.create = function (customer, name, river) {
  var s = Screen_Name.new(name);
  s.customer = customer;
  s.river = river;
  return s.create();
};

S.create.screen_name_insert_sql = '\
...
S.prototype.create = function () {
  ...
  var pg = Client.new(S.create.screen_name_insert_sql, [id, this.customer.data.id, me.sanitized_data.screen_name]);
  pg.run_and_on_finish(function (err, meta) {
    if (err) {
      if (err.detail && err.detail.match(/Key \(screen_name\)=/) && err.detail.match(/\) already exists/) )
        return me.river.invalid('Screen name already exists: ' + me.screen_name);
      else
        return me.river.error(err);
    }
...
Customer.prototype.read_homepage = function (name, river) {
  var me = this;
  pg.id_select( null, 'screen_names', 'id', me.screen_name_id(name), river);
};

Customer.prototype.read_screen_names = function (river) {
  var me = this;
...
Customer.prototype.update_screen_name = function ( name, new_vals, river ) {
  var me      = this;
...
Customer.prototype.undo_trash_screen_name = function (n, river) {
  var db = new pg.query();
  var me = this;
  db.on_error(river);
...
Customer.prototype.trash_screen_name = function (n, river) {
  var db = new pg.query();
  var me = this;
  db.on_error(river);
...
Customer.prototype.delete_screen_name = function (n, river) {
