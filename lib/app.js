...
app.post('/sign-in', function (req, resp, next) {
  ...
  passport.authenticate('local', function(err, user, info) {
    ...
    if (!user) {
      return resp.end(JSON.stringify({ msg: "Username/passphrase was wrong. Check your spelling.", success: false }));
    }

    req.login(user, function(err) {
      if (err)
        return next(err);
      resp.end(JSON.stringify({ msg: "You are now sign-ed in. Please wait as page reloads...", success: true }));
    });
  ...
  mem.create(function (meta) {
    if (meta.rowCount === 0) {
      ...
      req.login(mem, function(err) {
        ...
        return resp.end(JSON.stringify({ msg: "Account created.", success: true }));
      });
    }
  }, function (c) {
