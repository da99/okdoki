...
app.get('/info/:name/list/qa', function (req, resp, next) {
  ...
  Customer.read_by_screen_name(n, function (owner) {
    ...
    if (!priv.allow)
      ...
      owner.read_qa(n, req.user, function (rows) {
        if (rows.length === 0)
          return write_json(resp, {success: true, msg: "No questions/answers posted yet.", rows: [] });
        else
