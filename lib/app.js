...
var OST = parseInt((new Date(2013, 0, 31)).getTime());
function since_OST() {
...
function delete_old_msg(msgs_id, id, ost, r) {
  r.data.del_id_count += 1;

  if (!ost)
    r.data.del_ids.push(id);

  if(r.data.del_id_count < r.data.total_records)
    return null;

  if (r.data.del_ids.length === 0)
    return r.data.fin();

  r.add('delete ids from list (as hash fields)', 'hdel', [msgs_id, r.data.del_ids], r.data.fin);

}

function delete_old_msgs(msgs_id, prev_ids, r) {
  if (prev_ids.length === 0)
    return false;
  r.data.del_id_count = 0;
  r.data.total_records = prev_ids.length;
  _.each(prev_ids, function (id) {
    r.add('get_' + id, 'hget', [id, "at_OST"], function (reply, r) { delete_old_msg(msgs_id, id, reply, r); });
  });
}
...
app.post("/chat/heart-beep", require_log_in, function (req, resp, next) {
  ...
  client.data.fin = function () {
    return write_json_success(resp, "Fin.", obj);
  }

  if (dev_ip === req.ip && !req.body.message) {
    req.body.message = {message_body: 'No message.', owner_screen_name: req.user.data.screen_names[0]};
  }
  ...
  if (req.body.message) {
    ...
    client.data.wait_for_delete = true;

    client.data.msgs_id = msgs_id;

    // Get current messages to clear old ones.
    client.add('get old msgs' + msgs_id, 'hkeys', [msgs_id], function (reply, r) {
      delete_old_msgs(msgs_id, reply, r);
    });
    ...
    client.add('create msg: ' + m_id, 'hmset',  [m_id, m ], function () {
      client.add('msg expires: '+ m_id, 'expire', [m_id, 10]);
    });

    client.add('add msg id to list-as-hash: '    + m_id,    'hset',   [msgs_id, m_id, 1], function () {
      client.add('update expire of list-as-hash: ' + msgs_id, 'expire', [msgs_id, 10]);
    });

    obj.message = m;
  }

  // Save a record indicating screen name is online.
  _.each(user.data.screen_names, function (id, i) {
    client.add('user is online: ' + id, 'hmset' , [id, {s: since.toString()}], function () {
      client.add('user expires: ' + id  , 'expire', [id, 7]);
    });
  });

  client.exec(function (replies, r) {
    var msg = "Fin.";

    if (!client.data.wait_for_delete) {
      final();
    }
  });
  ...
  client.mget(contact_rids, function (err, result) {
    ...
    _.each(online_contacts, function (v, i) {
      ...
      _.each(o_menu, function (c_hash, o_name) {
        ...
        multi.hmset(o_name, o);
        multi.expire(o_name, 20);
      });

      // Insert owner into contact:
      multi.hmset(rid, contact_menu);
      multi.expire(rid, 20);
