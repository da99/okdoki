...
app.get('/info/:name/id/:id', function (req, resp) {
  var article = {
    ...
    'main_keyword' : 'jokes',
    'keywords' : req.param('name') + ' jokes ' + 'some keyword'
  };
...
function default_view_opts(name, req, resp) {
  var auth = true;
  return { homepage_belongs_to_viewer: false, 'view_name': name, logged_in: auth, screen_names: screen_names(['go99', 'Capt-Nando']), token: req.session._csrf};
}
