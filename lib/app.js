...
function write_json_fail(resp, msg, stat) {
  write_json(resp, {success: false, msg: msg}, (stat || 404));
}
...
app.get("/chat/heart-beep", require_log_in, function (req, resp, next) {
  var kvs   = []
  , expires = []
...
app.post("/contacts/online", require_log_in, function (req, resp, next) {
  ...
  var final = function (menu) {
    ...
    if (keys.length === 0) {
      return write_json(resp, {success: true, msg: "Fin.", menu: {}});
    }
    ...
    client.mget(keys, function (e, people_online_list) {
      ...
      return write_json_success(resp, "Fin.", {menu: o});

    });
  };

  client.get(id, function (err, mutual_contacts_online) {
