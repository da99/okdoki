...
app.get( '/info/:name', function (req, resp) {
  ...
  Customer.read_by_screen_name(n, function (owner) {
    ...
    var r = opts.screen_name_info;
    if (r.read_able === 'W' || opts.homepage_belongs_to_viewer)
        return resp.render(opts['view_name'], opts);

    if (r.trashed_at)
      return write_html(resp, '<html><body>Screen name not found: ' + n + '</body></html', 404);

    return write_html(resp, '<html><body>Screen name is private: ' + n + '</body></html');
...
app.post('/update/homepage/:name', function (req, resp, next) {
  var name = req.params.name;
  req.user.update_homepage(name, {about: req.body.about}, function (c) {
    write_json(resp, {success: true, msg: 'Your "About Me" section has been updated.'});
  });
});

app.post('/update/homepage/priv/:name', function (req, resp, next) {
  var n = req.params.name;
  var new_vals = {read_able: req.body.read_able, read_able_list: req.body.read_able_list };
  req.user.update_homepage(n, new_vals, function (c) {
    var msg = null;
