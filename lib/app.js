...
app.post("/chat/heart-beep", require_log_in, function (req, resp, next) {
  ...
  jobs.on_err(function (err, reply) {
    next(err);
  });

  _.each(req.user.data.screen_names, function (n, i) {

    var s = Redis_Screen_name.new(n, {on_err: err});

    jobs.create('refresh:', n, function (ff) {
      s.refresh(ff);
    });

    jobs.create_and_save('ims for:', n, function (ff) {
      s.read_messages(ff);
    });

    jobs.create('delete old ims for:', n, function (ff) {
      s.delete_old_ims(ff);
    });

    if (obj.message && obj.message.owner_screen_name === n) {
      jobs.create_and_save('create im', n, function (ff) {
        s.create_im(obj.message, ff);
      });
    }
  ...
  jobs.run(function () {
    obj.msgs    = _.compact(_.flatten(jobs.replys_for('ims for:'), 1));
    obj.message = jobs.replys_for('create im')[0];
