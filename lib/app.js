...
if (is_dev) {
  ...
  var fin = function (err, replys) {
    var last_cmd = cmd_q.shift();

    log( 'Error: ', err, 'Reply(s):', replys);
    ...
    if (cmd_q.length === 0) {
      if (status === 200)
        ...
        return write_json_fail(resp, "Fin.", status, obj);
    }
  }

  // For each screen name, update
  _.each(req.user.data.screen_names, function (n, i) {
    cmd_q.push("update online status for: " + n);
    var client = R.multi();

    client.hmset(n, {s: since});
    client.expire(n, secs + 7);
    client.exec(fin);
  ...
  cmd_q.push('save msg: ' + req.body.message);
  cmd_q.push('delete old msgs for: ' + o_name);
      ...
      R.hget(m_id, "at_OST", function (err, ost) {
        ...
        if (del_q < 1) {
          if (del_q_msg && del_q_msg.ids.length > 0) {
            console.log('dleeting: ', del_q_msg.ids.sort())
            console.log(msgs_id, del_q_msg.ids)
            R.hdel(msgs_id, del_q_msg.ids, fin);
          } else
            fin(null, replys);
        }
