...
app.post('/post/:name', function (req, resp, next) {
  ...
  .on_job('invalid', function (j) {
    write_json_fail(resp, j.errors.join(' '));
  })
...
app.post('/create-account', function (req, resp, next) {
  ...
  .run_and_then(function (r, save_customer) {
...
app.post('/create/screen_name', function (req, resp, next) {
  ...
  New_River(next)
  .on_job('invalid', function (j) {
    return write_json_fail(resp, j.errors.join(' '));
  })
  .job('create-screen-name', [Screen_Name, 'create', req.user, n])

  .run_and_then(function () {
...
app.post("/contacts/online", require_log_in, function (req, resp, next) {
  ...
  r.job('contact is online', req.user.data.id, [Contact, 'is_online', req.user]);
  r.run_and_then(function () {
