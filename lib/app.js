...
app.post("/chat/heart-beep", require_log_in, function (req, resp, next) {
  ...
  if (req.body.message) {
    ...
    client.hmset(m_id, m);
    client.expire(m_id, 10);

    client.hset(mgs_id, m_id, 1);
    client.expire(mgs_id, 10);
  ...
  client.exec(function (err, replies) {
    ...
    if (err) {
      log(err);
      return write_json_fail(resp, msg, 200, obj);
    }

    var prev_ids = (delete_old) ? replies[0] : [];


    if (prev_ids.length === 0)
      return write_json_success(resp, msg, obj);

    var client = redis_client.multi();

    _.each(prev_ids, function (id) {
      client.hget(id, "at_OST");
    ...
    client.exec(function (err, replies) {
      ...
      _.each(replies, function (ost, i) {
        if (!ost)
          del_ids.push(prev_ids[i]);
      });
      ...
      var client = redis_client.multi();
      client.hdel(delete_old, del_ids);
      client.exec(function (err, replies) {
        if(err)
          log(err);
        return write_json_success(resp, msg, obj);
