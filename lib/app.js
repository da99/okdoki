...
app.post('/sign-in', function (req, resp, next) {
  ...
  if (!req.body.screen_name || req.body.screen_name.trim().length == 0 )
    return write_json( resp, { msg: "Username is required.", success: false } );
  ...
  passport.authenticate('local', function(err, user, info) {
    ...
    if (!user) {
      return write_json( resp, { msg: "Username/passphrase was wrong. Check your spelling.", success: false } );
    }
