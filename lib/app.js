...
function delete_old_msg(id, ost, r) {
  if (!ost)
    r.data.del_ids.push(id);
}

function delete_old_msgs(prev_ids, r) {
  if (prev_ids.length === 0)
    return false;
  _.each(prev_ids, function (id) {
    r.add('get_' + id, 'hget', [id, "at_OST"], function (reply, r) { delete_old_msg(id, reply, r); });
  });
}
...
app.post("/chat/heart-beep", require_log_in, function (req, resp, next) {
  ...
  if (req.body.message) {
    ...
    client.add('get old msgs' + msgs_id, 'hkeys', [msgs_id], delete_old_msgs);
    ...
    client.add('create msg: ' + m_id, 'hmset', [ m_id, m ]);
    client.add('msg expires: '+ m_id, 'expire', [m_id, 10]);

    client.add('add msg id to list: ' + m_id, 'hset', [msgs_id, m_id, 1]);
    client.add('update expiration: ' + msgs_id, 'expire', [msgs_id, 10]);
  ...
  _.each(user.data.screen_names, function (id, i) {
    client.add('user is online: ' + id, 'hmset' , [id, {s: since}]);
  ...
  client.exec(function (replies, r) {
    var msg = "Fin.";
    if (r.data.del_ids) {
      r.add('delete ids from list (as hash fields)', 'hdel', [r.data.del_ids], final);
    } else
      final();
  });
