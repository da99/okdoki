...
app.get( '/info/:name', function (req, resp) {
  ...
  Customer.read_by_screen_name(n, function (owner) {
    ...
    if (opts.screen_name_info.is_trashed)
      opts.body_css.push('trashed');
    resp.render(opts['view_name'], opts);
  }, function () {
...
app.post('/trash/screen_name/:name', function (req, resp, next) {
  ...
  req.user.trash_screen_name(name, function () {
    write_json(resp, {success: true, msg: req.user.screen_name_row(name).trash_msg});
  });
});
