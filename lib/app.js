var new_pg_client = require('okdoki/lib/DB.js').new_pg_client;
...
app.post('/sign-in', function (req, resp) {
  ...
  if (!req.body.mask_name || req.body.mask_name.trim().length == 0 )
...
app.post('/create-account', function (req, resp) {
  ...
  var seed = (req.ip + (new Date).getTime()).replace( /[^0-9]/g, '');
  var mask_id            = shortid.seed(seed).generate();
  var consumer_id        = shortid.seed(seed).generate();
