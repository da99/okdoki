...
function write_json_fail(resp, msg, stat) {
  write_json(resp, {success: true, msg: msg}, (stat || 404));
}

function write_json_success(resp, msg) {
  write_json(resp, {success: true, msg: msg});
}
...
// ****************** CONTACTS ************************************
// ****************************************************************
app.get("/chat/heart-beep", function (req, resp, next) {
  var kvs   = []
  , expires = []
  ...
  client.exec(function (err, replies) {
    if (err)
      return next(err);
    write_json_success(resp, "Fin.");
  });
});

app.get("/contacts/online", function (req, resp, next) {
  var client = redis_client;
  var id = "oc:" + req.user.data.id;

  client.on("error", function (err) {
    next(err);
  });

  var log_it = function (err) {
    if (err)
      log("Redis error: " + err);
    return next(err);
  }

  var final = function (menu) {

    var keys = _.map(_.keys(menu), function (k) { return "o:" + k; });

    client.mget(keys, function (e, people_online_list) {
      ...
      _.each(people_online_list, function (v, i) {
        var name = v.replace('o:', '');
        if (menu[name])
          o[name] = menu[name];
      });

      write_json(resp, {success: true, msg: "Fin.", menu: o});
  ...
  client.get(id, function (err, reply) {
    ...
    if (mutual_contacts_online)
      final(mutual_contacts_online);
    else {
      req.user.read_mutual_contacts(function (rows, menu) {
        var o = {};
        _.each(menu, function (v, k) {
          o[k] = v.join(',');
        });
        client.hmset(id, o, function () { final(o) } );
      });
    }
