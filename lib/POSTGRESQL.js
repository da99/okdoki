...
exports.Client = function (q, vals) {
  this.query = q;
  this.vals  = vals;
};

exports.Client.prototype.run_and_on_finish = function (func) {
  var client = new pg_client();
  client.query(this.query, this.vals, function (err, meta) {
    client.end();
    func(err, meta);
  });
};

exports.Client.new = function (q, vals) {
  var c = new exports.Client(q, vals);
  return c;
};
...
query.prototype.ignore_next_duplicate = function (constraint) {
  this.dup_ignores.push('violates unique constraint "' + constraint + '"');
}
...
query.prototype.untrash = function (table, id, on_fin) {
  var db = this.q("UPDATE " + table + " SET trashed_at = null WHERE id = $1;", [id]);
...
query.prototype.run_next = function () {
  ...
  me.client.query(q[0], q[1], function (err, meta) {
    ...
    var ignore_dup = err &&
      me.dup_ignores.length > 0 &&
      err.message.indexOf('duplicate key value') > -1 &&
      err.message.indexOf(me.dup_ignores[0]) > 10;

    if (ignore_dup) {
      me.dup_ignores.shift();
      err = null;
    }
...
exports.generate_id = function (raw_seed) {
...
exports.id_select = function (db, table, id_col, id_val, on_fin, on_err) {
  ...
  q.q(s.sql, s.values);
  q.on_error(on_err);
  q.run_and_then( function (meta) {
    var row = meta.rows[0];
    if (!row) {
      var e = new Error('Not found in ' + table + ' for id: ' + id_val);
      if (on_err)
        return on_err(e);
      else
        throw e;
    ...
    _.each(['details', 'settings'], function (v, i) {
...
exports.sqler = function (sql, vals) {
  ...
  var s = sql.replace( exports.sqler.pattern, function (a) {
    ...
    if (_.isString(vals[i]) || _.isNumber(vals[i]) || _.isNull(vals[i])) {
      ...
      throw new Error('Unknown value type: ' + typeof(vals[i]));
