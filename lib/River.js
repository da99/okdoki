...
Job.new = function (vals) {
  ...
  me.finish = function () {
    ...
    if (me.returned_to_river)
      return null;

    if (arguments.length === 1) {
      var unk = arguments[0];
      if (unk && unk.has_error && unk.has_error()) {
        return me.finish(unk.about_error.type, unk.about_error.msg);
      } else
        me.reply(arguments[0]);
    }
    ...
    if (arguments.length > 1) {
      ...
      if (err_func) {
        ...
        return err_func({
          ...
          finish: function () {
            ...
            parent(me).finish.apply(parent(me), arguments);
        }});

      } else if (parent(me)) {

        return me.final_finish();
      ...
      if (me.has_error() || !me.FINISHS.length)
        return me.final_finish();
...
Job.prototype.reply = function (res) {
  this.replys.push(arguments);
  this.result = res;
  return this;
};
...
River.prototype.final_finish = function () {
  var me = this;
  if (me.is_fin)
    return null;

  if (!me.has_error() && me.ON_FINISH) {
  ...
  if (me.has_error()) {
    if (parent(me))
      return parent(me).finish(me.about_error.type, me.about_error.msg);
    else
...
River.prototype.finish = function (j, unk) {
  ...
  if (me.has_error() && !arguments.length && me.ON_ERROR[me.about_error.type]) {
    var func = me.ON_ERROR[me.about_error.type];
    me.ON_ERROR[me.about_error.type] = null;
    return func({
      river: me,
      finish: function () {
        if (parent(me) && arguments.length === 2)
          return parent(me).finish.apply(parent(me), arguments);

        if (parent(me) && arguments.length === 0 && me.has_error())
          return parent(me).finish(me.about_error.type, me.about_error.msg);
