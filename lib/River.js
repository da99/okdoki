...
var Job = function (vals) {
  ...
  me.topogo_client = me.river.topogo_client;

  me.finishs.push(function () {
...
River.prototype.emit = function (name, msg, job_or_me) {
  ...
  if (name === 'finish' && me.is_finish && !me.is_stopped) {
    me.stop();
  }
...
River.prototype.job_with_1_reply = function (group, id, f) {
  this.job(group, id, f);
  this.job('Require 1 ' + group + ' reply', id, function (j) {
    if (!j.river.last_reply())
      j.not_found(group + ', ' + id + ', not found.');
    j.finish(j.river.last_reply());
  });
