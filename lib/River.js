...
River.new = function () {
  ...
  while(args.length) {
    ...
    if (_.isString(temp)) {
      ...
      me.parent_job = temp;
    else if (temp.invalid || temp.error || temp.finish || temp.not_found)
      me.parent_job = temp;
    else
...
River.prototype.reply_s_for = function (group, id) {
  ...
  _.find(me.named_results, function (arr, name) {
    if (use_both) {
      if (name === group+':'+id) {
        replys = arr;
        return true;
...
River.prototype.finish = function () {
  ...
  if (this.is_stopped || (this.parent_job && (!this.parent_job.river || this.parent_job.river.is_finish)))
    return this;
  ...
  this.named_results[job.group + ':' + job.id] = replys;
