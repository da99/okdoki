...
Screen_names.prototype.new_screen_name = function (val) {
  ...
  if (!this.Customer.validator.has_errors()) {
    this.Customer.sanitized_data.screen_name = val;
  }
};
...
Screen_names.prototype.validate = function () {
  ...
  if (c.is_new) {
    ...
    me.new_screen_name(me.Customer.new_data.mask_name);
    ...
    me.new_screen_name(c.new_data.new_screen_name);
...
Screen_names.prototype.update = function (db) {
  if (!this.Customer.new_data.old_screen_name)
    return false;

  var me = this;
  var c = this.Customer;
  var screen_name_id = this.to_id(c.new_data.old_screen_name);
  var sql = pg.sql_gen.update('screen_names', {id: screen_name_id}, {screen_name: c.sanitized_data.screen_name});
