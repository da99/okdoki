...
A.url = function () {
  return "http://localhost:8529/_api" + _.toArray(arguments).join('/').replace(A.double_slashs, '/');
};
...
A.prototype.read_one_by_example = function (doc, flow) {
  var me = this;
  request.put({
    url: A.url("/simple/first-example"),
    json: true,
    body: {
      collection : me.name,
      example: doc
    ...
    if (data.code === 200)
      flow.finish(data.document || data.result);
    else
      flow.not_found(doc);
  }));
};
...
A.prototype.read_list = function (q, vars, flow) {
  var me = this;
  request.post({
    url: A.url("/cursor/"),
    json: true,
...
A.prototype.update = function (id, data, flow) {
  var me = this;
  request({
...
A.prototype.untrash = function (id, flow) {
  var me = this;
  River.new(arguments)
...
A.prototype.trash = function (id, flow) {
  var me = this;
  var _now = now();
  River.new(arguments)
  .job('update', function (j) {
...
A.prototype.del_list = function (q, vars, flow) {
  var me = this;
  River.new(arguments)
  .job('read', function (j) {
    me.read_list(q, vars, j);
  })
  .run();
};
