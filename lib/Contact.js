...
Contact.create = function (new_vals, flow) {
  ...
  .job('get existing contact if exists', function (j) {
    ...
    if (!contact)
      return j.finish([]);
    PG.new(j)
    .q( select(new_vals.from, contact.data.id) )
    .run();
  })
  ...
  .job('get reflected contact if exists', function (j) {
    ...
    if (!contact || existing)
      return j.finish([]);
    PG.new(j)
    .q( select(contact.data.id, new_vals.from) )
    .run();
  })
  ...
  .run_and_on_finish(function (r) {
    ...
    .values({from: new_vals.from, to: contact.data.id})
    .run();
  })
  ;
}; // end .create
...
Contact.read_list_of_owners_to_contact = function (customer, flow) {
  ...
  .job('get contacts', function (j) {
    ...
    .q(" \
       ...
       WHERE  contacts.from IN ( " + SQL.join_array_by_comma(owner_ids, vals) + " ) \
       AND    contacts.to =  $1  \
  ...
  var sql = " \
    SELECT    \
       ...
       (CASE WHEN last_seen_at >= ((now() AT TIME ZONE 'UTC') - '4 seconds'::interval)  \
        ...
        ELSE FALSE     \
       END) AS online  \
    FROM contacts INNER JOIN online_customers                                  \
         (ON contacts.to = online_customers.screen_name_id)                    \
