...
Contact.create = function (new_vals, flow) {
  ...
  .job('get existing contact if exists', function (j, last) {
    contact = last;
    if (!contact)
      return j.finish([]);
  ...
  .job('get reflected contact if exists', function (j, rows) {
    existing = rows[0];
    if (!contact || existing)
      return j.finish([]);
  ...
  .job('save to vars', function (j, rows) {

    reflected = rows[0];

    if (!contact || (contact.data.read_able != 'W' && !reflected))
      return j.invalid("Screen_Name not found: " + new_vals.contact_screen_name);
    ...
    var insert_vals = {id: UID.generate_id(from_id + to_id), from_id: from_id, to_id: to_id};
    PG.new(j)
