...
T.prototype.run = function (q, vars, flow) {
  var me = this;
  var client = Client.new(me.db);
...
T.prototype.create = function (data, flow) {
  ...
  request.post({
    url: T.url('/document?collection=' + name),
    json: true,
    body: data
  }, on_complete(flow));
};
...
T.prototype.read_one_by_example = function (doc, flow) {
  var me = this;
  request.put({
    url: T.url("/simple/first-example"),
    json: true,
    body: {
      collection : me.name,
      example: doc
