...
IM.create = function (o, flow) {
  ...
  if (im.from_id() !== '[OKDOKI]')
    flow.error('Not ready to handle dims from actual users because' +
                    ' you need to verify the two customers know each other.');

  River.new(flow)
  .job('read screen_name', function (j) {
    Screen_Name.read_by_id(im.from, j)
  })
  .job('save im', function (j) {
  ...
  .run_and_on_finish(function (r) {
    flow.finish( IM.new(r.last_reply()) );
  });
    ...
    FROM ims                                        \
        ...
        LEFT JOIN screen_names to_sn                \
          ON ims.to_id = to_sn.id                   \
                                                    \
    WHERE (ims.from = '[OKDOKI]' OR ims.from_id IN (SELECT * FROM allowed_contacts))  \
          AND ims.created_at <= ((now() AT TIME ZONE 'UTC') - '" + IM.expire_in + " seconds'::INTERVAL);  \
  ";
