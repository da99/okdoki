...
IM.flow_defaults = function (flow) {
  if (!flow.error)
    flow.error = function (e) { throw e; };
  return flow;
};
...
IM.read = function (name, flow) {
  var me = this;
  var ims = [];
  var accumulate = {
    error: flow.error,
    finish: function (replys) {
      ims.push(replys);
      if (ims.length !== 2)
        return;
      var sorted = _.flatten(ims, 1).sort(function (a,b) {
  ...
  IM.read_ims(name, accumulate);
  IM.read_dims(name, accumulate);
};

IM.read_dims = function (n, flow) {
  IM.flow_defaults(flow);

  var me = this;
  var new_message = [];

  Redis.pop_records_from_list(n + ':dims', function (e, r){
    if (e)
      return flow.error(e, r);
    flow.finish(r);
  });
};

IM.read_ims = function (n, flow) {
  IM.flow_defaults(flow);
  ...
  Redis.client.hkeys(n + ':c', function (err, contacts) {
    if (err)
      return flow.error(err, contacts);
    ...
    multi.exec(function (err, raw_ids) {
      Redis.read_records_from_ids(_.flatten((raw_ids || []), 1), function (err, ims) {
        if (err)
          return flow.error(err, ims);
        flow.finish(ims);
      });
