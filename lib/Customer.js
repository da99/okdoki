...
Customer.prototype.create = function (func, on_err) {
  ...
  db.q('BEGIN;');
  db.q("INSERT INTO customers (id, passphrase_hash) VALUES ( $1, encode(digest( $2, 'sha512'), 'hex') );", [customer_id, customer_id + passphrase]);
  ...
  if (_.isString(customer_id)) {
    ...
    db.q(" \
      ...
      WHERE id IN (SELECT customer_id FROM screen_names WHERE screen_name = $1 LIMIT 1) \
        AND passphrase_hash = encode(digest( id || $2, 'sha512' ), 'hex') \
      LIMIT 1; \
