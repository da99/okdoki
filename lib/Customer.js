...
Customer.prototype.validate = function (vals) {
  ...
  if (this.is_new) {
    ...
    if (this.new_data.hasOwnProperty('old_screen_name'))
      this.push_sanitized_data('screen_name_id', this.screen_name_id(this.new_data.old_screen_name));

    if (this.new_data.hasOwnProperty('screen_name'))
      this['new_screen_name'](this.new_data.screen_name);

    if (this.new_data.hasOwnProperty('homepage_about'))
      this['edit_homepage_about'](this.new_data.homepage_about);

    if (this.new_data.hasOwnProperty('homepage_title'))
      this['edit_homepage_title'](this.new_data.homepage_title);
...
Customer.prototype.create = function (func, on_err) {
  ...
  db.q('INSERT INTO homepages (screen_name_id, settings, details) VALUES ($1, $2, $3);', [screen_name_id, '', '']);
  db.q('COMMIT;');
...
Customer.prototype.create_screen_name = function (name, on_fin, on_err) {
  ...
  db.q('INSERT INTO homepages (screen_name_id, settings, details) VALUES( $1, $2, $3);', [id, '', '']);
  db.q('COMMIT;');
...
Customer.prototype.read_homepage = function (name, on_fin, on_err) {
  ...
  pg.id_select( null, 'homepages', 'screen_name_id', id, function (meta) {
    var row = meta.rows[0];
    if (row) {
      return on_fin(row);
    }

    var e = new Error('Homepage not found for: ' + name);
    if (!on_err)
      return on_err(e);
    else
      throw e;
...
Customer.prototype.update_homepage = function ( name, new_vals, on_fin, on_err ) {
  ...
  if (d.hasOwnProperty('homepage_title')) {
    v_sql += " details = details ||  ('title' => ~x) "
    v_arr.push(d.homepage_title);
  }
