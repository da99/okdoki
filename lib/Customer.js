...
Customer.prototype.validate = function (vals) {
  ...
  if (this.is_new) {
    ...
    var keys = "passphrase confirm_passphrase ip screen_name".split(' ');
    ...
    _.each( ['screen_name', 'email', 'about', 'homepage_title', 'homepage_allow', 'read_able', 'read_able_list'], function (k, i) {
...
Customer.prototype.read_qa = function (n, aud, on_fin, on_db_err) {
  var me              = this;
  var n_meta          = me.screen_name_row(n);
  var viewed_by_owner = (aud && aud.data.id === me.data.id)
  var vals            = [n_meta.id, Customer.to_section_id('qa')];
  var sql     = " \
     SELECT * \
     FROM posts \
     WHERE pub_id = $1 \
