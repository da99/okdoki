...
Customer.prototype.read_feed = function (on_fin) {
  ...
  var sql = "SELECT * \
    ...
    WHERE       \
      ...
      AND \
      NOT (json_get_varchar_array(settings, 'disallow') && ARRAY(SELECT label_id FROM labelings WHERE pub_id IN (SELECT id FROM screen_names WHERE customer_id = $1))::varchar[])  \
    LIMIT 10;";
  var db  = new pg.query();
  db.q(sql, [me.data.id, "@"]);
  db.run_and_then(function (meta) {
