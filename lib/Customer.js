...
Customer.prototype.validate = function (o) {
  ...
  if (this.is_new) {
    ...
    if (!keys) {
      keys = ['screen_name', 'email', 'about', 'homepage_title', 'homepage_allow', 'read_able', 'read_able_list'];
      prefix = 'edit_';
    }
...
Customer.prototype.new_passphrase = function (v) {
  var new_v = (v || "").trim();
  this.validator.check(new_v, 'Passphrase must be at least 9 chars long.').is(/^.{9,}$/i);
  this.push_sanitized_data('passphrase', new_v);
};
...
Customer.prototype.create = function (func, on_err) {
  ...
  this.data.screen_names   = [screen_name];
...
Customer.prototype.create_screen_name = function (name, on_fin, on_err) {
  ...
  db.on_error(rollback(db.client, new_on_err));
...
Customer.prototype.create_question = function (vals, on_fin, on_invalid) {
  var me = this;
  me.new_data = vals;
  ...
  var seed     = parseFloat(raw_seed.replace( /[^0-9]/g, ''));
  var post_id  = shortid.seed(seed).generate();
  ...
  var sql_vals = [
    ...
    Customer.to_section_id('question'),
...
Customer.read_by_screen_name = function ( n, on_fin, on_not_found ) {
  var db = new pg.query();
  db.q('SELECT * FROM screen_names WHERE screen_name = $1;', [n]);
  db.run_and_then(function (meta) {
    var r = meta.rows[0];
    if (r) {
      Customer.read(r.customer_id, function (c, c_meta) {
...
Customer.prototype.read = function (customer_id, func, on_err) {
  ...
  if (_.isString(customer_id)) {
    ...
    db.q(" \
      SELECT * \
      FROM customers \
      WHERE id IN (SELECT customer_id FROM screen_names WHERE screen_name = $1 LIMIT 1) \
...
Customer.prototype.read_posts = function (type, n, aud, on_fin, on_db_err) {
  ...
  var vals            = [n_meta.id];
  var sql     = " \
     SELECT id, pub_id, body, created_at, trashed_at \
     FROM   posts \
     WHERE  pub_id = $1 \
  ";
  ...
  if (!viewed_by_owner) {
    vals.push('W');
    sql += (" AND read_able = $" + vals.length.toString() + " ");
  }
...
Customer.prototype.read_feed = function (on_fin) {
  ...
  var sql = "SELECT * \
    ...
    WHERE       \
      ...
      AND \
       ( allow && array_cat(ARRAY[$2]::varchar[], ARRAY(SELECT id FROM screen_names WHERE customer_id = $1))::varchar[] \
          OR  \
         allow && ARRAY(SELECT label_id FROM labelings WHERE pub_id IN (SELECT id FROM screen_names WHERE customer_id = $1))::varchar[] \
       ) \
      AND \
      NOT (disallow && array_cat(ARRAY[$2]::varchar[], ARRAY(SELECT id FROM screen_names WHERE customer_id = $1))::varchar[]) \
      AND \
      NOT (disallow && ARRAY(SELECT label_id FROM labelings WHERE pub_id IN (SELECT id FROM screen_names WHERE customer_id = $1))::varchar[])  \
