    ...
    WHERE       \
      ...
      AND \
       ( json_get_varchar_array(settings, 'allow') && array_cat(ARRAY[$2]::varchar[], ARRAY(SELECT id FROM screen_names WHERE customer_id = $1))::varchar[] \
          OR  \
         json_get_varchar_array(settings, 'allow') && ARRAY(SELECT json_get_varchar_array(details, 'labels') FROM contacts WHERE owner_id = contact_id IN (SELECT id FROM screen_names WHERE customer_id = $1)) \
       ) \
      AND \
