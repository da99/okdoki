    ...
    db.run_and_then(function (meta) {
      ...
      db.run_and_then(function (meta) {
        ...
        c_db.q("CREATE TABLE IF NOT EXISTS homepages ( \
               screen_name_id  varchar(" + Customer.id_size + ") PRIMARY KEY NOT NULL UNIQUE, \
...
Customer.prototype.create_screen_name = function () {};
  ...
  db.run_and_then(function (meta) {
    if (meta.rowCount === 0) {
      if (on_err)
        ...
        throw new Error('No screen names found for: ' + me.data.id);
    } else {
