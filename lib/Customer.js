...
Customer.prototype.create = function (func, on_err) {
  ...
  db.q('INSERT INTO screen_names (id, owner_id, screen_name) VALUES ($1, $2, upper($3));', [screen_name_id, customer_id, screen_name]);
  db.q('COMMIT;');
...
Customer.prototype.create_screen_name = function (name, on_fin, on_err) {
  ...
  db.q('INSERT INTO screen_names (id, owner_id, screen_name) VALUES ($1, $2, $3);', [id, me.data.id, me.sanitized_data.screen_name]);
...
Customer.prototype.read = function (customer_id, func, on_err) {
  ...
  if (_.isString(customer_id)) {
    ...
    db.q(" \
      SELECT * \
      FROM customers \
      WHERE id IN (SELECT owner_id FROM screen_names WHERE screen_name = $1 LIMIT 1) \
...
Customer.prototype.read_mutual_contacts = function (on_fin) {
  ...
  FROM contacts \
  WHERE owner_id IN \
    (SELECT id FROM screen_names WHERE owner_id = $1) \
  ";
  var vals = [me.data.id];
};
