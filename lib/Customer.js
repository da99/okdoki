...
Customer.prototype.screen_name_row = function (raw_name) {
  ...
  var r = _.find(rows, function (row) {
...
Customer.delete_trashed = function (flow) {
  var sql = "SELECT id \n" +
    ...
    "ELSE false END";
  PG.new('delete old records')
  .q("DELETE FROM " + Screen_Name.TABLE_NAME + " WHERE owner_id IN (" + sql + ") RETURNING owner_id ;")
  .q("DELETE FROM " + Customer.TABLE_NAME   + " WHERE id in (" + sql + ") RETURNING id ;")
