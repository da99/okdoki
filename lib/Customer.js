...
Customer.create = function (new_vals, flow) {
  ...
  .job('create', 'screen name', function (j) {
    Screen_Name.create(me.sanitized_data.screen_name, me, j);
  })
  ...
  .job('create', 'customer record', function (j) {
    ...
    .q(SQL.insert_into(TABLE_NAME).values({
      id: customer_id,
      pass_phrase: ['encode_pass_phrase( $1 )', customer_id + me.sanitized_data.pass_phrase]
    }))
...
console.log(c_flow.first_reply())

    me.push_screen_name_row(c_flow.first_reply());
    flow.finish(me);
  });
...
Customer.read_by_screen_name = function ( n, flow ) {
  var db = new pg.query();
  ...
  db.run_and_then(function (meta) {
    ...
    if (r) {
      Customer.read(r.owner_id, function (c, c_meta) {
        ...
        flow.finish(c, c_meta);
      });
    } else {
      flow.invalid(["Not found: " + n]);
    }
  });
};

Customer.read_by_id = function (id, flow) {
  var mem = new Customer();
  return mem.read(id, flow);
};

Customer.prototype.read = function (customer_id, flow) {
  ...
  db.run_and_then(function (meta) {
    ...
    if (meta.rowCount === 0)
      return flow.error(new Error('Customer not found: ' + (customer_id || screen_name)));

    if (meta.rowCount != 1)
      return flow(new Error('Unknown error: Trying to read customer: ' + (customer_id || screen_name)));
...
Customer.prototype.read_contacts_for = function (screen_name_id, flow) {
  var me = this;
  var db = new pg.query();
...
Customer.prototype.read_mutual_contacts = function (flow) {
  var me  = this;
  var db  = new pg.query();
  var sql = "\
SELECT \
...
Customer.prototype.update = function (new_vals, flow) {
  var me = this;
...
Customer.prototype.update_contacts = function (new_vals, flow) {
  var me         = this;
...
Customer.prototype.trash = function (flow) {
  var db = new pg.query();
  db.on_error(flow);
  db.trash('customers', this.data.id, flow);
};

Customer.prototype.delete = function (flow) {
