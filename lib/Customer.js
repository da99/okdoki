...
Customer.prototype.validate = function (vals) {
  ...
  while (keys.length !== 0) {
    k = keys.shift();
    if (this.new_values.hasOwnProperty(k))  {
      this[op + k](this.new_values[k]);
    }
  };
...
Customer.prototype.update_email = function (raw_v) {
  ...
  if (v.length === 0)
    this.sanitized_new_values.email = null;
  else
    this.sanitized_new_values.email = v;
}
...
Customer.prototype.update = function (new_vals, func) {
  this.new_values = new_vals;
  var me = this;
  this.validate();
  if (this.is_valid) {
    var sql = squel.update().table('customers').where('id = $1');
    _.each(me.sanitized_new_values, function (v, k) {
      sql.set(k, v);
    });
