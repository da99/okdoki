...
Customer.prototype.update_contacts = function (new_vals, on_fin) {
  ...
  var sql = "UPDATE contacts \
           ...
           WHERE owner_id NOT IN ( $1 ) AND contact_id = $2;";
  ...
  Customer.read_by_screen_name(new_vals.contact_screen_name, function (c) {
    ...
    if (new_vals.as.length > 0) {

      _.each( new_vals.as, function (n) {

        var owner_id = me.screen_name_id(n);
      ...
      db.q(sql, [owner_ids, contact_id]);

    }

    if (owner_ids.length === 0) {
      db.q(sql.replace('NOT IN', 'IN'), [me.data.screen_names, contact_id]);
    }
