...
Customer.read_by_id = function (customer_id, flow) {

  var me = new Customer();

  River.new(flow)
  .job('read customer', customer_id, function (j) {
    PG.new('read customer id: ' + customer_id, j)
    .q(SQL.select('*').from(TABLE_NAME).where('id', customer_id).limit(1))
  ...
  .job('read screen names', customer_id, function (j) {
