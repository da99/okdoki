...
Customer.read_by_screen_name = function (opts, flow) {
  ...
  River.new(arguments)
  .job_with_1_reply('Screen_name', opts.screen_name, function (j) {
    Screen_Name.read_by_screen_name(j.id, j);
  })
  .job_with_1_reply('Customer', opts.screen_name, function (j) {
...
Customer.read_by_id = function (opts, flow) {
  ...
  .job_with_1_reply('Customer', screen_name || opts.id, function (j) {
    Topogo.new(TABLE_NAME)
    .read_one_by_example(opts, j);
  })
...
Customer.prototype.trash = function (flow) {
  var me = this;
  Topogo.new(TABLE_NAME)
  .trash(me.data.id, flow.concat(function (row) {
    me.data.trashed_at = row;
    flow.finish(me);
  }));
};
