...
CB.send_ims = function (on_fin) {
  var mr = Mail_Room.new(on_fin);
  mr.read_im();
};
...
CB.prototype.ims_fail = function (arr_ims) {
  ...
  _.each(msgs, function (ims, name) {
    var vals = {msg: "Try again later.", msgs: msgs, category: "could not send"};
    Screen_Name.new(name).create_sys_im(val, me.on_fin);
  });
};
